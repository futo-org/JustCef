...
{
public:
    explicit BinaryResourceProvider(const std::string& root_url) : root_url_(root_url) { DCHECK(!root_url.empty()); }
    ...
    bool OnRequest(scoped_refptr<CefResourceManager::Request> request) override
    {
        ...
        {
            CefRefPtr<CefStreamReader> stream = GetResourceReader(relative_path.data());
            if (stream.get())
            {
