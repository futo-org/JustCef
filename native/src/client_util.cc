...
{
public:
    explicit PendingFileDialogRequest(int browser_identifier) : browser_identifier_(browser_identifier) {}
...
{
public:
    PendingPathsDialogRequest(int browser_identifier, std::function<void(std::vector<std::string>)> callback)
        : PendingFileDialogRequest(browser_identifier), callback_(std::move(callback))
    {
    }
    ...
    void Complete(const std::vector<CefString>& file_paths)
    {
        ...
        callback_(std::move(paths));
    }

private:
    void OnCancel() override { callback_({}); }
...
{
public:
    PendingPathDialogRequest(int browser_identifier, std::function<void(std::string)> callback)
        : PendingFileDialogRequest(browser_identifier), callback_(std::move(callback))
    {
    }
    ...
    void Complete(const std::vector<CefString>& file_paths)
    {
        ...
        callback_(!file_paths.empty() ? file_paths.front().ToString() : std::string());
    }

private:
    void OnCancel() override { callback_(std::string()); }
...
void RemovePendingFileDialogRequest(const std::shared_ptr<PendingFileDialogRequest>& request)
{
    ...
    requests.erase(std::remove_if(requests.begin(), requests.end(),
                                  [&](const std::shared_ptr<PendingFileDialogRequest>& candidate)
                                  {
...
{
public:
    explicit FileDialogPathsCallback(std::shared_ptr<PendingPathsDialogRequest> request) : request_(std::move(request)) {}
...
{
public:
    explicit FileDialogPathCallback(std::shared_ptr<PendingPathDialogRequest> request) : request_(std::move(request)) {}
...
void RunFileDialogForPathsOnUi(std::shared_ptr<PendingPathsDialogRequest> request, int browser_identifier, CefBrowserHost::FileDialogMode mode, const std::string& title,
                               const std::string& default_file_path, std::vector<CefString> accept_filters)
{
...
void RunFileDialogForPathOnUi(std::shared_ptr<PendingPathDialogRequest> request, int browser_identifier, CefBrowserHost::FileDialogMode mode, const std::string& title,
                              const std::string& default_file_path, std::vector<CefString> accept_filters)
{
    CEF_REQUIRE_UI_THREAD();
...
void PlatformPickFiles(int browser_identifier, bool multiple, const std::vector<std::pair<std::string, std::string>>& filters,
                       std::function<void(std::vector<std::string>)> callback)
{
    auto request = std::make_shared<PendingPathsDialogRequest>(browser_identifier, std::move(callback));
...
{
    auto request = std::make_shared<PendingPathDialogRequest>(browser_identifier, std::move(callback));
    ...
    else if (!CefPostTask(TID_UI, base::BindOnce(&RunFileDialogForPathOnUi, request, browser_identifier, FILE_DIALOG_OPEN_FOLDER, std::string("Select Directory"), std::string(),
                                                 std::move(filters))))
    {
        RemovePendingFileDialogRequest(request);
        request->Cancel();
                      ...
                      std::function<void(std::string)> callback)
{
    ...
    else if (!CefPostTask(TID_UI, base::BindOnce(&RunFileDialogForPathOnUi, request, browser_identifier, FILE_DIALOG_SAVE, std::string("Save File"), default_name,
                                                 std::move(accept_filters))))
    {
        RemovePendingFileDialogRequest(request);
        request->Cancel();
    }
